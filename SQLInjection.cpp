...
//
/*  Updated by Connie Knupp on 7/12/2025 for CS-405 */
...
{
    // Completed: Fix this method to fail and display an error if there is a suspected SQL Injection
    //  NOTE: You cannot just flag 1=1 as an error, since 2=2 will work just as well. You need
    //  something more generic

    //create lowercase copy of sql string to negate any case changes
    std::string lowercase_string = sql;
    std::transform(lowercase_string.begin(), lowercase_string.end(), lowercase_string.begin(), [](unsigned char c) { return std::tolower(c); });

    //search sql string for or statement in lowercase
    size_t pos = lowercase_string.find("' or ");
    //if found output error and cancel query
    while (pos != std::string::npos) {
        std::cout << "WARNING: SQL injection attack detected!  Query cancelled\n";
        return false;
    }
